...
This has been an additive process over the history of SPIFFE. The standard was originally written with X.509-SVID as the only supported format, and JWT-SVID was added subsequently to aid in interoperability with Layer 7 networking.
...
This section provides a brief description of the available SVID formats for contextual and comparative clarity. Full specification details for each can be found in their corresponding standardisation (linked) documents.
...
Casting SVID information to an X.509 format allows for transport-level encryption to be used between peers with TLS capabilities, as well as fufiling the identity requirements of SPIFFE. 
...
JWT-SVIDs are bearer tokens, and do not enable any kind of encryption or protection against impersonation. Various recommendations exist to mitigate against known vulnernabilities of the JWT as an identity token, and should be followed when JWT-SVIDs are used.
...
This section provides guidance on scenarios where a single SVID format can be considered favourable when compared with the alternatives. This should not be considered exhaustive, and there are likely to be cases where multiple or any of the formats could be argued to be suitable.
...
* All workloads have TLS capabilities (either natively or via proxying)

### 4.2 When to use JWT-SVID
JWT-SVIDs are the preferred choice for systems where application layer authentication is required (in contrast to trasport layer). This might apply when:
...
WIT-SVIDs are the preferred choice for systems where application layer authentication is required (as with JWT-SVID), but there is an additional requirement for stronger security assurances. Specifically, this will be in cases where:

* Workloads need to safeguard against impersonation more strongly than with token expiry,
