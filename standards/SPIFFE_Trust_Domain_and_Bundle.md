...
The `keys` parameter may contain an empty array. Clients may also encounter bundles which after processing yield no usable keys (i.e. no JWKs pass validation described below), and are effectively empty.  In both cases, workloads MUST treat all SVIDs from the trust domain as invalid.  SPIFFE control planes that process bundles MAY wish alert operators when they encounter actually empty or effectively empty bundles. The former indicates that the trust domain is no longer issuing SPIFFE identities, while the latter may indicate that the trust domain has migrated to a new key type or use not understood by the client.
